...
public class TabLayout extends HorizontalScrollView {
    ...
    public void setupWithTitles(List<String> titles) {
        if (titles != null) {
            removeAllTabs();

            for (int i = 0; i < titles.size(); i++) {
                addTab(newTab().setText(titles.get(i)), false);
            }
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        public void draw(Canvas canvas) {
            ...
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                if (mSelectedIndicatorWidth <= 0 || (mSelectedIndicatorWidth >= (mIndicatorRight - mIndicatorLeft))) {
                    canvas.drawRect(mIndicatorLeft, getHeight() - mSelectedIndicatorHeight,
                            mIndicatorRight, getHeight(), mSelectedIndicatorPaint);
                } else {
                    int left = mIndicatorLeft + (mIndicatorRight - mIndicatorLeft - mSelectedIndicatorWidth) / 2;
